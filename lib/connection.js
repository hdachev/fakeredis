...
exports.Connection = function ( backend, minLatency, maxLatency )
{
    ...
    function exec ()
    {
        ...
        if ( q ) while (( entry = q.shift () ))
        {
            ...
            //console.log ( ID, '<exec> ' + JSON.stringify ( entry ) );
    ...
    this.pushMessage = function ( type, channel, message )
    {
        ...
        //console.log ( ID, state === BLOCKED, type, channel.toString (), message );

        // if ( state !== BLOCKED && type !== 'unsubscribe' )
            // throw new Error ( "WOOT! Connection:pushMessage when not blocked : " + JSON.stringify ( arguments ) );

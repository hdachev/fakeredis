...
, SCAN: function(){
	if (arguments.length < 1) return BAD_ARGS;

	var cursor = parseInt(arguments[0], 10);
	var match = arguments[1];
	var matchPattern = arguments[2];
	var count = arguments[3];
	var countNum = parseInt(arguments[4], 10);
	var keys;

	// Return a random num of results between 1 and 10 to approximate
	// real-life redis usage
	var keyCount = (count && countNum)
					? countNum
					: Math.round(Math.random()*9);

	keys = this.getKeys().slice(cursor, cursor+keyCount);

	// Reached the end of keys; return cursor 0
	if (this.getKeys().length <= cursor+keyCount && !keys.length) {
		return [0];
	}

	// Apply MATCH filtering _after_ getting number of keys
	if (match && matchPattern){
		keys = keys.filter(pattern(matchPattern));
	}

	return [cursor + keyCount, keys];
}
